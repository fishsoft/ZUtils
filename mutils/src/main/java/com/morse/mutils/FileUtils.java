...
public class FileUtils {
	...
	private static final int IS_DIRECTORY_PATH = 1;
	private static final int NOT_IS_DIRECTORY_PATH = 0;
	...
	public static long formatFileSize(String size) {
		if (size.contains("G"))
			return (long) (Float.parseFloat(size.substring(0, size.indexOf("G"))) * GB);
		else if (size.contains("M"))
			return (long) (Float.parseFloat(size.substring(0, size.indexOf("M"))) * MB);
		else if (size.contains("K"))
			return (long) (Float.parseFloat(size.substring(0, size.indexOf("K"))) * KB);
		return 0;
	}
	 ...
	 * @return 返回文件扩展名
	 */
	public static String get﻿ExtensionName﻿(String filename) {
